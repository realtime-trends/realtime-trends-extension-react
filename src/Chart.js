...
function Chart(props) {
  ...
  useEffect(() => {
    if(!handlerRef.current) {
      cachedFetch('/api/issue', 300).then(data => data.json()).then(data => {
        setIssueList(JSON.parse(data).issue_list);
      });
      setInterval(() => {
        cachedFetch('/api/issue', 300).then(data => data.json()).then(data => {
          setIssueList(JSON.parse(data).issue_list);
  ...
  return (
    ...
    {issueList && issueList.map((issue, index) => {
        const labelId = `label-${issue.num}`;
        if (index < 10) {
      return (
      <TableRow key={issue.num} role={undefined} onClick={event => query(event, defaultEngine, issue.word)} hover={true}>
      <TableCell component="th" scope="row">
        <Typography style={{ 'fontWeight': (index === bold) ? 'bold' : 'normal', 'fontSize': '12px' }}>{issue.num}.</Typography>
        </TableCell>
      <TableCell scope="row">
      <TextTransition
        text={"#"+issue.word}
    ...
    <TableCell scope="row">
      {issue.delta === 999 ? <FiberNewIcon style={{ color: 'orange' }} /> :
      issue.delta > 0 ? <div style={{
        display: 'flex',
        alignItems: 'center',
        flexWrap: 'wrap',
    }}><ArrowDropUpIcon style={{ color: 'red' }} /><span>{Math.abs(issue.delta)}</span></div>  :
        issue.delta < 0 ? <div style={{
          display: 'flex',
          alignItems: 'center',
          flexWrap: 'wrap',
      }}><ArrowDropDownIcon style={{ color: 'blue' }} /><span>{Math.abs(issue.delta)}</span></div>  : 
      ...
      return (
          <Chip size="small" label={"#"+issue.word} component="a" onClick={event => query(event, defaultEngine, issue.word)} />
      );
    }}
      )}
       </TableBody>
